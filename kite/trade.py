...
scrips = ['NFO:BANKNIFTY23AUGFUT',
          'NFO:NIFTY23AUGFUT']

today = datetime.datetime.now()
from_d = today- datetime.timedelta(days =2)


print(scrips[1])
hist_data = session.historical_data(8963586, from_d, today, interval='5minute')
for pr in hist_data:
    price_data.loc[pd.to_datetime(pr['date']),['open','high','low','close','volume']] = [pr['open'],pr['high'],pr['low'],pr['close'],pr['volume']]
...
while True:
    ...
    price_data1.loc[pytz.utc.localize(pd.to_datetime(qt["NFO:BANKNIFTY23AUGFUT"]['timestamp'])),['close']] = qt["NFO:BANKNIFTY23AUGFUT"]['last_price']
    # print('quote')
    # print(price_data1)
    print('hist')
    # price_data = price_data[['close']]
    print(price_data.tail(4))
    five_min_candles = price_data1.resample('5T').agg({'close': 'ohlc'})
    # print(five_min_candles)
    five_min_candles = price_data.append(five_min_candles['close'])
    ...
    # five_min_candles = five_min_candles['close']
    # print(five_min_candles.tail(4))
